...
public class SceneLevelMap {
    ...
    private static void load(LevelEvent.Load event) {
        ...
        for(Map.Entry<ResourceKey<SceneType<?, ?>>, SceneType<?, ?>> entry : BBRegistries.SCENE_TYPE.get().entrySet()) {
    ...
    private static void tick(LevelTickEvent.Pre event) {
        for(Scene<?> scene : scenes) {
            if(scene.sceneType().tickPre()) scene.tickInternal();
        }
    }
    ...
    private static void tick(LevelTickEvent.Post event) {
        for(Scene<?> scene : scenes) {
            if(scene.sceneType().tickPost()) scene.tickInternal();
        }
    }
    ...
    private static void chunkUnload(ChunkEvent.Unload event) {
        ...
        for(Scene<?> scene : Theatre.get(level)) {
            //Babel.LOGGER.info("{} {} {} {}", chunk.getPos(), scene.getSceneType().getKey(), scene.dimension(), scene.level());
