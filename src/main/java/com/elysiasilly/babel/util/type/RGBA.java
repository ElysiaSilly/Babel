package com.elysiasilly.babel.util.type;
...
public class RGBA {
    ...
    private static int cast(float f) {
        return UtilsMath.castToRange(0, 1, 0, 255, f);
    }
    ...
    public RGBA shade(int shade) {
        return new RGBA(round((float) r() / shade), round((float) g() / shade), round((float) b() / shade), a());
    }
    ...
    public static final Codec<RGBA> CODEC = UtilsCodec.RGBA_CODEC;

    public static final StreamCodec<ByteBuf, RGBA> STREAM_CODEC = UtilsCodec.RGBA_STREAM_CODEC;
    ...
    public static final RGBA YELLOW = new RGBA(1f, 1f, 0f);
    public static final RGBA CYAN = new RGBA(0f, 1f, 1f);
    public static final RGBA PURPLE = new RGBA(1f, 0f, 1f);
}
